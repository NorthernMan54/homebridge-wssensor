...
function Accessory(params) {
  ...
  this.name = this.accessoryDef.Hostname;
  this.service_name = this.accessoryDef.Model;
...
Accessory.prototype.save_and_setValue = function (trigger, c, value) {
  ...
  //debug("Accessory.save_and_setValue %s %s %s", trigger, c, value);
  ...
  if (result.isValid) {
    ...
    //debug("Accessory.save_and_setValue %s %s %s %s %s ", trigger, this.name, c, result.value, JSON.stringify(context));
...
Accessory.prototype.addService = function(newAccessory) {
  ...
  if (typeof(Service[this.service_name]) !== "undefined") {
    ...
    isValid = true;
    //debug("Accessory.addService %s", JSON.stringify(service, null, 2));

  } else {
    //debug("Accessory.addService service '%s' undefined.", this.service_name);
    isValid = false;
  }
  return isValid;
}
...
Accessory.prototype.configureAccessory = function(accessory) {
  ...
  //debug("Accessory.configureAccessory %s %s %s\n", this.name, this.service_name, JSON.stringify(this.service.characteristics));

  debug("Accessory.configureAccessory Def", JSON.stringify(this.accessoryDef),JSON.stringify(this.service));

  var c;
  for (var k in this.service.characteristics) {

    c = this.service.characteristics[k].displayName.replace(/\s/g, "");
    debug("Accessory.configureAccessory %s %s %s", this.name, this.service_name, c);
    ...
    if (c != "Name") {
      ...
      debug("Accessory.configureAccessory %s %s %s %s", this.name, this.service_name, c, JSON.stringify(this.i_props));
    }
  }
  ...
  for (var k in this.service.optionalCharacteristics) {
    ...
    if (typeof(this.accessoryDef[c]) !== "undefined") {
      debug("Accessory.configureAccessory %s %s optional %s", this.name, this.service_name, c);
...
Accessory.prototype.allocate = function(c) {
  ...
  if (sc.props.perms.indexOf("pw") > -1) {
    //debug("Accessory.allocate 'set' event %s %s", this.name, c);
...
Accessory.prototype.setProps = function(c) {
  ...
  if (typeof(this.accessoryDef[c]) !== "undefined") {
    ...
    //debug("Accessory.setProps %s %s", this.name, c, this.accessoryDef[c]);
    //debug("Accessory.setProps %s %s", this.name, c, Characteristic[c]);
  }
}
...
Accessory.prototype.get = function(callback, context, displayName) {
  ...
  debug("Accessory.get %s %s", this.name, c);
  ...
  debug("Accessory.get %s %s %s", this.name, c, value);
  callback(null, value);
}
...
Accessory.prototype.set = function(value, callback, context, displayName) {
  ...
  var c = displayName.replace(/\s/g, "");
  //debug("Accessory.set %s %s %s %s", this.name, c, value, JSON.stringify(context));
  ...
  if (typeof(context) !== "undefined" && typeof(context.trigger) !== "undefined" && context.trigger.match(/websocket/g)) {
    //debug("Accessory.set %s %s %s - websocket", this.name, c, value);
    callback();
  } else {
